...
export const styleIndividualButtons = (button: CopyButtonElement, config: Config): void => {
	const copysvg =
		config.iconsvg?.copy && config.iconsvg.copy !== ""
			? config.iconsvg.copy
			: original_icons.copy;
	const copiedsvg =
		config.iconsvg?.copied && config.iconsvg.copied !== ""
			? config.iconsvg.copied
			: original_icons.copied;
	...
	if (button.textholder) {
		button.textholder.textContent = button.dataset.ccCopy
			? button.dataset.ccCopy
			: config.text?.copy || "Copy";
	}
};
