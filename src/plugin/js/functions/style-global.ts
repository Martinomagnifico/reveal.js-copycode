...
export const applyCustomStyles = (revealEl: HTMLElement, config: Config): void => {
	...
	if (
		isDifferentFromDefault(config.style.copyborder, defaultConfig.style.copyborder) &&
		config.style.copyborder !== ""
	) {
		revealEl.style.setProperty("--cc-copyborder", config.style.copyborder);
	}

	if (
		isDifferentFromDefault(config.style.copiedborder, defaultConfig.style.copiedborder) &&
		config.style.copiedborder !== ""
	) {
